...
int n, m, k;
int board[1000][1000];
int dis[1000][1000][10][2]; // 0 낮 1 밤
...
int main()
{
	...
	while (check_que.size())
	{
		...
		if (sel_x == n - 1 && sel_y == m - 1) // 탈출
		{
			cout << dis[sel_x][sel_y][sel_cnt][sel_isnight] << '\n';
			return 0;
		}
			// for문에서 바로 체크했지만 혹시나 밤낮이 문제인가 싶어서 위로 따로 뺌
			// ↑ 이새끼가 범인이엿음 ㅋㅋ, 출구에 벽이 잇는경우도 생각해볼것
		...
		for (int dir = 0; dir < 4; dir++)
		{
			...
			if (board[nx][ny] == 0) // 빈칸일떄
			{
				if (dis[nx][ny][sel_cnt][1 - sel_isnight] > 0) continue;

				dis[nx][ny][sel_cnt][1 - sel_isnight] = dis[sel_x][sel_y][sel_cnt][sel_isnight] + 1;
				check_que.push({nx, ny, sel_cnt, 1 - sel_isnight});
			}
			else // 벽일때
			{
				if (sel_cnt >= k) continue;

				if (sel_isnight) // 밤일때
				{
					if (dis[sel_x][sel_y][sel_cnt][1 - sel_isnight] > 0) continue;

					dis[sel_x][sel_y][sel_cnt][1 - sel_isnight] = dis[sel_x][sel_y][sel_cnt][sel_isnight] + 1;
					check_que.push({sel_x, sel_y, sel_cnt, 1 - sel_isnight});
				}
				else // 낮일떄
				{
					if (dis[nx][ny][sel_cnt + 1][1 - sel_isnight] > 0) continue;

					dis[nx][ny][sel_cnt + 1][1 - sel_isnight] = dis[sel_x][sel_y][sel_cnt][sel_isnight] + 1;
					check_que.push({nx, ny, sel_cnt + 1, 1 - sel_isnight});
				}
			}
		}
	}
...
// 대충 밤낮의 상태를 배열로 가지는게 아니라 queue에서만 넣어주던 코드
...
// 			{
// 				if (sel_cnt >= k || dis[nx][ny][sel_cnt + 1][1] > 0 || dis[nx][ny][sel_cnt + 1][0] > 0) continue; // 더이상 부술수없거나 방문했으면 continue

// 				if (sel_isnight) // 밤일때 - select의 같은위치의 낮 배열에 +1해서 넘겨줌
// 				{
// 					dis[sel_x][sel_y][sel_cnt][0] = dis[sel_x][sel_y][sel_cnt][sel_isnight] + 1;
// 					check_que.push({sel_x, sel_y, sel_cnt, 0});
// 				}
// 				else // 낮일때 - nx ny의 위치 밤 배열에 +1 해줌
// 				{
